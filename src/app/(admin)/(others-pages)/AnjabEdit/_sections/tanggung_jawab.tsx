...
export default function TanggungJawabForm({
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                const res = await apiFetch(`/api/anjab/${encodeURIComponent(id)}/tanggung-jawab`, { cache: "no-store" });
                if (!alive) return;
    ...
    async function saveRow(idx: number) {
        ...
        try {
            ...
            if (it.id_tanggung_jawab > 0) {
                res = await apiFetch(`/api/anjab/${encodeURIComponent(id)}/tanggung-jawab/${it.id_tanggung_jawab}`, {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
            } else {
                res = await apiFetch(`/api/anjab/${encodeURIComponent(id)}/tanggung-jawab`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
    ...
    async function deleteRow(idx: number) {
        ...
        try {
            if (it.id_tanggung_jawab > 0) {
                const res = await apiFetch(`/api/anjab/${encodeURIComponent(id)}/tanggung-jawab/${it.id_tanggung_jawab}`, {
                    method: "DELETE",
                });
