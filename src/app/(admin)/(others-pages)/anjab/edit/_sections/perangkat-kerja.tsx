// src/app/(admin)/(others-pages)/AnjabEdit/_sections/perangkat-kerja.tsx
"use client";
...
type PerangkatKerjaRow = {
    id: number;                     // SERIAL dari backend (kolom "id")
    jabatan_id: string;             // UUID (kolom "jabatan_id")
    perangkat_kerja: string[];
    penggunaan_untuk_tugas: string[];
    _tmpKey?: string;               // untuk key React lokal
};
...
export default function PerangkatKerjaForm({ viewerPath }: { viewerPath: string }) {
    ...
    const fetchAll = async (jabatanId: string) => {
        ...
        setLoading(true);
        try {
            const res = await apiFetch(`/api/anjab/${encodeURIComponent(jabatanId)}/perangkat-kerja`, { cache: "no-store" });
            if (!res.ok) {
                setRows([]);
                setLastError(`Gagal memuat Perangkat Kerja (HTTP ${res.status}).`);
                return;
            }
            const raw = await res.json();
            const normalized: PerangkatKerjaRow[] = Array.isArray(raw)
    ...
    function addRow() {
        ...
        setRows(prev => [
            ...prev,
            { id: 0, jabatan_id: resolvedId, perangkat_kerja: [""], penggunaan_untuk_tugas: [], _tmpKey: tmpKey },
        ]);
        setTimeout(() => firstRef.current?.focus(), 0);
    }
        ...
        try {
            if (it.id > 0) {
                ...
                const res = await apiFetch(
                    `/api/anjab/${encodeURIComponent(resolvedId)}/perangkat-kerja/${it.id}`,
                ...
                updateLocal(idx, {
                    ...
                    perangkat_kerja: Array.isArray(json.data?.perangkat_kerja) ? json.data.perangkat_kerja : [],
                    penggunaan_untuk_tugas: Array.isArray(json.data?.penggunaan_untuk_tugas) ? json.data.penggunaan_untuk_tugas : [],
                });
            } else {
                // POST
                const res = await apiFetch(
                    `/api/anjab/${encodeURIComponent(resolvedId)}/perangkat-kerja`,
    ...
    // Jika UUID tidak ada di localStorage → persempit ke pesan teks (mirip Jabatan/Unit Kerja)
    ...
    if (loading) {
        return (
            <EditSectionWrapper
                title="Perangkat Kerja"
                description="Memuat data perangkat kerja..."
                icon={
    ...
    return (
        <EditSectionWrapper
            title="Perangkat Kerja"
            description="Edit perangkat kerja dan penggunaan untuk tugas dalam jabatan ini"
            icon={
                <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
            ...
            <div className="space-y-6">
                {rows.length === 0 ? (
                    <div className="text-center py-12 px-4 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-4">Belum ada perangkat kerja yang ditambahkan</p>
                        <button
                            type="button"
                            onClick={addRow}
                    ...
                    <>
                        {rows.map((row, idx) => {
                            ...
                            return (
                                <FormSection key={key} title={`Perangkat Kerja ${idx + 1}`}>
                                    <DualList
                                        ...
                                        leftPlaceholder="Contoh: Laptop, Printer"
                                        rightPlaceholder="Contoh: Penyusunan dokumen"
