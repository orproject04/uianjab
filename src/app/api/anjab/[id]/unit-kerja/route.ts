...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    try {
        ...
        const { rows } = await pool.query(
            `SELECT id_unit_kerja, id_jabatan, jpt_utama, jpt_madya, jpt_pratama,
                    administrator, pengawas, pelaksana, jabatan_fungsional,
...
export async function HEAD(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    try {
        const { id } = await ctx.params;
        const { rows } = await pool.query(`SELECT 1 FROM unit_kerja WHERE id_jabatan = $1`, [id]);
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    try {
        ...
        const up = await pool.query(
            `UPDATE unit_kerja
             SET jpt_utama=$1, jpt_madya=$2, jpt_pratama=$3,
                 administrator=$4, pengawas=$5, pelaksana=$6,
                 jabatan_fungsional=$7, updated_at=NOW()
             WHERE id_jabatan=$8`,
