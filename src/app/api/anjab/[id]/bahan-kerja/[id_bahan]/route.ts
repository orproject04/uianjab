// src/app/api/anjab/[id]/bahan-kerja/[id_bahan]/route.ts
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_bahan: string }> }) {
    try {
        ...
        if (!isIntId(id_bahan)) return NextResponse.json({error: "Invalid, id_bahan harus angka"}, {status: 400});
        ...
        if (!up.rowCount) return NextResponse.json({error: "Not Found, (Bahan Kerja tidak ditemukan)"}, {status: 404});
        ...
        if (e?.code === "22P02") {
            return NextResponse.json({error: "Invalid, id harus UUID, id_bahan harus angka"}, {status: 400});
        }
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string; id_bahan: string }> }) {
    try {
        ...
        if (!del.rowCount) return NextResponse.json({error: "Not Found, (Bahan Kerja tidak ditemukan)"}, {status: 404});
