...
export async function PATCH(
    ...
    try {
        ...
        if (!isIntId(id_bahan)) {
            return NextResponse.json({error: "id_bahan harus integer > 0"}, {status: 400});
        }
        const bid = Number(id_bahan);
        if (!(bid > 0)) {
            return NextResponse.json({error: "id_bahan harus > 0"}, {status: 400});
        }
        ...
        if (e?.code === "22P02") {
            return NextResponse.json({error: "jabatan_id harus UUID / id_bahan harus int"}, {status: 400});
        }
