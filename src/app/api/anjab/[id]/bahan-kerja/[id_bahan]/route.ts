...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_bahan: string }> }) {
    try {
        ...
        const fields: string[] = [];
        const values: any[] = [];
        if (p.data.bahan_kerja !== undefined)             { fields.push(`bahan_kerja=$${fields.length + 1}`); values.push(p.data.bahan_kerja); }
        if (p.data.penggunaan_dalam_tugas !== undefined)  { fields.push(`penggunaan_dalam_tugas=$${fields.length + 1}`); values.push(p.data.penggunaan_dalam_tugas); }

        if (!fields.length) return NextResponse.json({ ok: true });

        values.push(id, bid);
        const q = `UPDATE bahan_kerja SET ${fields.join(", ")}, updated_at=NOW()
                   WHERE id_jabatan=$${fields.length + 1} AND id_bahan=$${fields.length + 2}`;
        ...
        const { rows } = await pool.query(
            `SELECT id_bahan, id_jabatan, bahan_kerja, penggunaan_dalam_tugas, created_at, updated_at
             FROM bahan_kerja WHERE id_jabatan=$1 AND id_bahan=$2`,
            [id, bid]
        );
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string; id_bahan: string }> }) {
    try {
        const { id, id_bahan } = await ctx.params;
        const bid = Number(id_bahan);
        const del = await pool.query(`DELETE FROM bahan_kerja WHERE id_jabatan=$1 AND id_bahan=$2`, [id, bid]);
