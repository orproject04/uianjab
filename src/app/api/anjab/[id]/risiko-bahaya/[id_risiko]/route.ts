...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_risiko: string }> }) {
    try {
        ...
        if (p.data.nama_risiko !== undefined) {
            fields.push(`nama_risiko=$${fields.length + 1}`); values.push(p.data.nama_risiko);
        }
        if (p.data.penyebab !== undefined) {
            fields.push(`penyebab=$${fields.length + 1}`); values.push(p.data.penyebab);
        }

        if (!fields.length) return NextResponse.json({ ok: true });

        values.push(id, rid);
        const q = `UPDATE risiko_bahaya SET ${fields.join(", ")}, updated_at=NOW()
               WHERE id_jabatan=$${fields.length + 1} AND id_risiko=$${fields.length + 2}`;
       ...
       FROM risiko_bahaya WHERE id_jabatan=$1 AND id_risiko=$2`,
            [id, rid]
        );
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string; id_risiko: string }> }) {
    try {
        ...
        const del = await pool.query(
            `DELETE FROM risiko_bahaya WHERE id_jabatan=$1 AND id_risiko=$2`,
            [id, rid]
        );
