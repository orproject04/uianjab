...
const TextRequired = z.union([z.string(), z.number()]).transform(v => String(v).trim()).refine(s => s.length > 0, "Field wajib diisi.");
// text → trim (opsional)
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    try {
        ...
        const data = rows.map((r: any) => ({ ...r, dalam_hal: Array.isArray(r.dalam_hal) ? r.dalam_hal : [] }));
        ...
        const ins = await client.query(
            `INSERT INTO korelasi_jabatan
             ...
             VALUES ($1, $2, $3, $4, NOW(), NOW())
                 RETURNING id, jabatan_id, jabatan_terkait, unit_kerja_instansi, dalam_hal, created_at, updated_at`,
...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        await client.query("BEGIN");
        await client.query(`DELETE FROM korelasi_jabatan WHERE jabatan_id = $1`, [id]);
