...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_tanggung_jawab: string }> }) {
    try {
        ...
        values.push(id, tid);
        const q = `UPDATE tanggung_jawab SET ${fields.join(", ")}, updated_at=NOW()
               WHERE id_jabatan=$${fields.length + 1} AND id_tanggung_jawab=$${fields.length + 2}`;
       ...
       FROM tanggung_jawab WHERE id_jabatan=$1 AND id_tanggung_jawab=$2`,
            [id, tid]
        );
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string; id_tanggung_jawab: string }> }) {
    try {
        ...
        const del = await pool.query(
            `DELETE FROM tanggung_jawab WHERE id_jabatan=$1 AND id_tanggung_jawab=$2`,
            [id, tid]
        );
