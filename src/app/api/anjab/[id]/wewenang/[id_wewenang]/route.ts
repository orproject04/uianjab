...
const TextField = z.union([z.string(), z.number()]).transform(v => String(v).trim()).refine(s => s.length > 0, "Uraian wewenang wajib diisi.");
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_wewenang: string }> }) {
    try {
        ...
        const { id, id_wewenang } = await ctx.params; // id = jabatan_id (UUID), id_wewenang = SERIAL di URL
        const wid = Number(id_wewenang);
        if (!Number.isFinite(wid) || wid <= 0) return NextResponse.json({ error: "Not Found" }, { status: 404 });
