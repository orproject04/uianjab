...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_wewenang: string }> }) {
    try {
        ...
        values.push(id, wid);
        const q = `UPDATE wewenang SET ${fields.join(", ")}, updated_at=NOW()
               WHERE id_jabatan=$${fields.length + 1} AND id_wewenang=$${fields.length + 2}`;
       ...
       FROM wewenang WHERE id_jabatan=$1 AND id_wewenang=$2`,
            [id, wid]
        );
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string; id_wewenang: string }> }) {
    try {
        ...
        const del = await pool.query(
            `DELETE FROM wewenang WHERE id_jabatan=$1 AND id_wewenang=$2`,
            [id, wid]
        );
