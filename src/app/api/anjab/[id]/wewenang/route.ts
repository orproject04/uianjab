...
const ItemSchema = z.object({ uraian_wewenang: TextField });
    ...
    try {
        ...
        const ins = await client.query(
            `INSERT INTO wewenang (jabatan_id, uraian_wewenang, created_at, updated_at)
             VALUES ($1, $2, NOW(), NOW())
                 RETURNING id, jabatan_id, uraian_wewenang, created_at, updated_at`,
...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        await client.query("BEGIN");
        await client.query(`DELETE FROM wewenang WHERE jabatan_id = $1`, [id]);
