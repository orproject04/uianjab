...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_tugas: string }> }) {
    ...
    try {
        ...
        if (p.data.nomor_tugas !== undefined) {
            fields.push(`nomor_tugas=$${fields.length + 1}`);
            values.push(p.data.nomor_tugas);
        }
        if (p.data.uraian_tugas !== undefined) {
            fields.push(`uraian_tugas=$${fields.length + 1}`);
            values.push(p.data.uraian_tugas);
        }
        if (p.data.hasil_kerja !== undefined) {
            fields.push(`hasil_kerja=$${fields.length + 1}`);
            values.push(p.data.hasil_kerja);
        }
        if (p.data.jumlah_hasil !== undefined) {
            fields.push(`jumlah_hasil=$${fields.length + 1}`);
            values.push(p.data.jumlah_hasil);
        }
        if (p.data.waktu_penyelesaian_jam !== undefined) {
            fields.push(`waktu_penyelesaian_jam=$${fields.length + 1}`);
            values.push(p.data.waktu_penyelesaian_jam);
        }
        if (p.data.waktu_efektif !== undefined) {
            fields.push(`waktu_efektif=$${fields.length + 1}`);
            values.push(p.data.waktu_efektif);
        }
        if (p.data.kebutuhan_pegawai !== undefined) {
            fields.push(`kebutuhan_pegawai=$${fields.length + 1}`);
            values.push(p.data.kebutuhan_pegawai);
        }
