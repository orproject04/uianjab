...
const TextRequired = z
    ...
    .refine(s => s.length > 0, "Aspek wajib diisi.");
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string; id_kondisi: string }> }) {
    try {
        ...
        if (p.data.aspek !== undefined) {
            fields.push(`aspek=$${fields.length + 1}`); values.push(p.data.aspek);
        }
        if (p.data.faktor !== undefined) {
            fields.push(`faktor=$${fields.length + 1}`); values.push(p.data.faktor);
        }

        if (!fields.length) return NextResponse.json({ ok: true });

        values.push(id, kid);
        const q = `UPDATE kondisi_lingkungan_kerja SET ${fields.join(", ")}, updated_at=NOW()
               WHERE id_jabatan=$${fields.length + 1} AND id_kondisi=$${fields.length + 2}`;
        const up = await pool.query(q, values);
        if (!up.rowCount) return NextResponse.json({ error: "Not Found" }, { status: 404 });

        const { rows } = await pool.query(
            `SELECT id_kondisi, id_jabatan, aspek, faktor, created_at, updated_at
       FROM kondisi_lingkungan_kerja WHERE id_jabatan=$1 AND id_kondisi=$2`,
            [id, kid]
        );
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string; id_kondisi: string }> }) {
    try {
        ...
        const del = await pool.query(
            `DELETE FROM kondisi_lingkungan_kerja WHERE id_jabatan=$1 AND id_kondisi=$2`,
            [id, kid]
        );
