...
const SELECT_ANJAB_BY_ID = `
  ...
  FROM jabatan j
    ...
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('hasil_kerja', h.hasil_kerja,'satuan_hasil', h.satuan_hasil)) AS hasil_kerja
      FROM hasil_kerja h WHERE h.id_jabatan = j.id_jabatan
    ) h ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('bahan_kerja', b.bahan_kerja,'penggunaan_dalam_tugas', b.penggunaan_dalam_tugas)) AS bahan_kerja
      FROM bahan_kerja b WHERE b.id_jabatan = j.id_jabatan
    ) b ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('perangkat_kerja', p.perangkat_kerja,'penggunaan_untuk_tugas', p.penggunaan_untuk_tugas)) AS perangkat_kerja
      FROM perangkat_kerja p WHERE p.id_jabatan = j.id_jabatan
    ) p ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('uraian_tanggung_jawab', t.uraian_tanggung_jawab)) AS tanggung_jawab
      FROM tanggung_jawab t WHERE t.id_jabatan = j.id_jabatan
    ) t ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('uraian_wewenang', w.uraian_wewenang)) AS wewenang
      FROM wewenang w WHERE w.id_jabatan = j.id_jabatan
    ) w ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('jabatan_terkait', kj.jabatan_terkait,'unit_kerja_instansi', kj.unit_kerja_instansi,'dalam_hal', kj.dalam_hal)) AS korelasi_jabatan
      FROM korelasi_jabatan kj WHERE kj.id_jabatan = j.id_jabatan
    ) kj ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('aspek', k.aspek,'faktor', k.faktor)) AS kondisi_lingkungan_kerja
      FROM kondisi_lingkungan_kerja k WHERE k.id_jabatan = j.id_jabatan
    ) klk ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_AGG(JSON_BUILD_OBJECT('nama_risiko', r.nama_risiko,'penyebab', r.penyebab)) AS risiko_bahaya
      FROM risiko_bahaya r WHERE r.id_jabatan = j.id_jabatan
    ) rb ON TRUE
    LEFT JOIN LATERAL (
      SELECT JSON_BUILD_OBJECT(
        'keterampilan_kerja', s.keterampilan_kerja,'bakat_kerja', s.bakat_kerja,'temperamen_kerja', s.temperamen_kerja,'minat_kerja', s.minat_kerja,'upaya_fisik', s.upaya_fisik,
        'kondisi_fisik_jenkel', s.kondisi_fisik_jenkel,'kondisi_fisik_umur', s.kondisi_fisik_umur,'kondisi_fisik_tb', s.kondisi_fisik_tb,'kondisi_fisik_bb', s.kondisi_fisik_bb,'kondisi_fisik_pb', s.kondisi_fisik_pb,
        'kondisi_fisik_tampilan', s.kondisi_fisik_tampilan,'kondisi_fisik_keadaan', s.kondisi_fisik_keadaan,'fungsi_pekerja', s.fungsi_pekerja
      ) AS syarat_jabatan
      FROM syarat_jabatan s WHERE s.id_jabatan = j.id_jabatan
    ) sj ON TRUE
    LEFT JOIN LATERAL (
      SELECT COALESCE(
        JSON_AGG(
          JSON_BUILD_OBJECT(
            'id_tugas', tp.id_tugas,'nomor_tugas', tp.nomor_tugas,'uraian_tugas', tp.uraian_tugas,'hasil_kerja', tp.hasil_kerja,'jumlah_hasil', tp.jumlah_hasil,
            'waktu_penyelesaian_jam', tp.waktu_penyelesaian_jam,'waktu_efektif', tp.waktu_efektif,'kebutuhan_pegawai', tp.kebutuhan_pegawai,
            'tahapan', (
              SELECT COALESCE(JSON_AGG(JSON_BUILD_OBJECT('id_tahapan', tut.id_tahapan, 'tahapan', tut.tahapan)), '[]')
              FROM tahapan_uraian_tugas tut WHERE tut.id_tugas = tp.id_tugas
            )
          ) ORDER BY tp.nomor_tugas
        ),
        '[]'
      ) AS tugas_pokok
      FROM tugas_pokok tp WHERE tp.id_jabatan = j.id_jabatan
    ) tp ON TRUE
