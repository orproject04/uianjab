...
export type AnjabRow = {
    id_jabatan: string;                 // alias dari j.id (UUID)
...
const SELECT_ANJAB_BY_ID_OR_SLUG = `
    SELECT
        j.id                                   AS id_jabatan,
        j.kode_jabatan,
        j.nama_jabatan,
        ...
        j.slug,                                -- hanya 1 slug sekarang

        u.jpt_utama, u.jpt_madya, u.jpt_pratama,
        u.administrator, u.pengawas, u.pelaksana, u.jabatan_fungsional,
        ...
        SELECT COALESCE(
                       JSON_AGG(
                               JSON_BUILD_OBJECT(
                                       ...
                                       ( SELECT COALESCE(
                                                        JSON_AGG(
                                                                JSON_BUILD_OBJECT('id_tahapan', tut.id, 'tahapan', tut.tahapan)
                                                                    ORDER BY tut.id
                                                        ), '[]'
                                                )
...
export async function getAnjabByIdOrSlug(idOrSlug: string): Promise<AnjabRow | null> {
    const { rows } = await pool.query<AnjabRow>(SELECT_ANJAB_BY_ID_OR_SLUG, [idOrSlug]);
    return rows[0] ?? null;
}