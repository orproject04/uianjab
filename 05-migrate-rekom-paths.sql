-- Update existing rekom_jf paths to use /api/storage/ prefix
-- This is needed to serve files through Next.js API route instead of static files

UPDATE rekom_jf
SET kemenpan_path = REPLACE(kemenpan_path, '/storage/', '/api/storage/')
WHERE kemenpan_path IS NOT NULL 
  AND kemenpan_path LIKE '/storage/%';

UPDATE rekom_jf
SET instansi_pembina_path = REPLACE(instansi_pembina_path, '/storage/', '/api/storage/')
